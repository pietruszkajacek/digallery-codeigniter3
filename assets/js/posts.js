	...
	posts: {
		init: function () {
			...
			$("#posts-compose-message").submit(function () {
				...
				if ($('textarea[name="post_message"]').val() == "")
				{
					...
					msg_error = msg_error + 'Wiadomość musi zawierać treść.' + '<br />';
				}
				else
				{
				...
				DIGALLERY.common.alert('#compose-message', 'Błąd', msg_error, 'error');
				
				return false;
			});
			
			// wyświetla podpowiedzi adresów email użytkowników
			$('#recipient').typeahead({
				minLength: 3,
		...
		show_post_in: function () {
			...
			$("#posts-message-inbox").submit(function () {
				if ($('input[name="subject"]').val() != "" && $('textarea[name="post_message"]').val() != "")
				{
					return true;
				}
